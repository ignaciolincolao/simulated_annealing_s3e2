...
std::tuple<double, int> sasFunc(float lenTemp1,
    ...
    int level_registers) // Si es 0, no imprimira ni almacenara, si es 1, imprimira por pantalla solo resultados, si es 2 almacenara los resultados en un archivo y mostrara su progreso, si es 3 se registra en detalle cada movimiento.
{
    ...
    n_block = n_blo;
    n_thread = n_thr;
    coolingRate = k_cooling;
    len1 = lenTemp1;
    len2 = lenTemp2;
    k_reheating = k_reheating_var;
    n_reheating = n_reheating_var;
    max_reheating = max_reheating_var;
    temp = temp_init;
    min_temp =temp_min;
    if(init_seed < 0){
        mt.seed(time(NULL));
